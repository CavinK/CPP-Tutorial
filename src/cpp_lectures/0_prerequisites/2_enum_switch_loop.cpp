...
열거체는 숫자에 이름을 붙여주는 기능이다. <- 플래그!
코드 가독성을 높이기 위한 거!
*/
enum NUM
{
	NUM_0 = 10, // 아무 것도 부여하지 않을 경우 0부터 1씩 차례로 값이 부여된다.
	NUM_1,
	NUM_2 = 0x0010, // 16
	NUM_3 // 17
};

// 전처리기(#) 사용 <- 컴파일 타임에 값을 정해놓는 것
#define NUM_4 4
...
int main() 
{
	...
	switch문으로 쓸 수 있는 건 if문으로도 가능, but switch문은 조건을 체크하지는 못 함
	...
	*/
	...
	case NUM_1: // iNumber 값이 무엇인지에 따라서 실행되는 case 구문이 결정된다.
		...
		break; // break를 만나게 되면 switch문을 빠져나가게 된다. <- break가 없을 경우 바로 아래에 있는 case 구문도 강제로 실행되게 된다.
	case NUM_2:
	...
	case NUM_4:
		...
		break;
	default: // case로 지정되어 있지 않은 숫자가 들어올 경우 실행된다 <- if문의 else에 대응!
	...
	//NUM eNum;
	//NUM eNum = 10; // 에러! <- 10은 int 타입이고 eNum은 NUM 타입
	NUM eNum = (NUM)10; // type casting

	// sizeof(타입 or 변수) 를 하게 되면 해당 타입 혹은 변수의 메모리 크기를 구해준다.
	cout << sizeof(NUM) << endl;
	// typeif(타입 or 변수).name() 을 하게 되면 typeid 안에 들어간 타입 혹은 변수의 타입을 문자열로 반환해준다.
	// 이게 어떤 타입인 지 알 수 있게 문자 타입으로 반환!
	...
	*/
	...
	while (false) // true
	{
		...
		if (iPlayer < SRP_Scissor || iPlayer > SRP_END)
		{
	...
	조건식이 true이면 코드 블럭의 코드가 동작된다.
	무한루프를 돌릴 때는 for(;;) {} 을 해주면 무한으로 돌아가게 된다.
	for문에서 초기값은 처음 for문에 진입할 때 딱 1번만 동작된다.
	그 후에 조건식을 체크하고 조건식이 true이면 코드 블럭의 코드가 동작된 후에 증감값을 처리한다.
	그 후 다시 조건을 체크하고 true면 동작되고 증감 -> 조건 -> 증감 -> 조건의 순서로 처리가 되다가,
	조건이 false이거나 break를 만나게 되면 for 문을 빠져나가게 된다.
	*/
	...
	// 1은 10보다 작으므로 true가 나오고 1 출력 ++해서 2가 되고 조건 true
